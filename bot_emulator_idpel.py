...
def main():
    ...
    for row in range(2, sheet.max_row + 1):
        ...
        # karena ini baca lewat idpel maka ini tidak diperlukan
        ...
        if status_exist in ["SUKSES", "Error : SUDAH TERCATAT PADA SISTEM FASIH", "Error : Nik tidak valid", "Error : data alamat server kosong", "Error: Alamat NULL", "Error : Alamat NULL", "Error : id pelanggan tidak ditemukan", "Error : nama tidak murni alphabeth"]:
        ...
        try:
            ...
            # NAVIGASI DASHBOARD
            # ==========================================
            # === CEK APAKAH SUDAH BERADA DI FORM INPUT ID PELANGGAN ===
            ...
            if not is_on_form:
                ...
                # === LANGKAH 7: SCROLL KE TOMBOL CEK ID PELANGGAN ===
                print("[BLOK I] Men-scroll secara dinamis ke tombol Cek ID Pelanggan...")
                d(scrollable=True).scroll.to(text="Cek ID Pelanggan")
                time.sleep(SLEEP_SHORT)
            else:
                print("[INFO] Sudah berada di form pengisian ID Pelanggan. Melewati Langkah 3, 4, 5 & 7.")

            # === LANGKAH 6: INPUT ID PELANGGAN ===
            ...
            if cek_id_btn.exists(timeout=5):
                t_start = time.time()
                cek_id_btn.click()
                print(f"[{datetime.datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [BLOK I] Mengetuk tombol 'Cek ID Pelanggan'...")
                ...
                time.sleep(0.1) # Beri jeda agar progress bar sempat muncul
                ...
                # Cek apakah limit API check-idpln tercapai (langsung via .exists())
                limit_msg = "Permintaan API check-idpln sudah terlampaui (limit)."
                if d(textContains=limit_msg).exists():
                    print(f"[{datetime.datetime.now().strftime('%H:%M:%S.%f')[:-3]}] PROSES BOT BERHENTI KARENA BATAS CEK ID PELANGGAN MELEWATI LIMIT")
                ...
                print(f"[{datetime.datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Cek status target selesai (status_found={status_found}, tahap ini: +{t_status_end - t_status_start:.2f}s, total: +{t_status_end - t_start:.2f}s)")
                
                if not status_found:
                    # Pengecekan kondisi jika ID Pelanggan sudah terdaftar
                    ...
                    # Pengecekan kondisi jika ID Pelanggan tidak ditemukan / status invalid
                    is_not_found = (
                    ...
                    if is_already_registered:
                        t_done = time.time()
                        print(f"[{datetime.datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [BLOK I] Baris {row} | IDPEL {idpel}: Pelanggan SUDAH TERDAFTAR di FASIH (+{t_done - t_start:.2f}s). Lanjut ke IDPEL berikutnya...")
                    ...
                    if is_not_found:
                        t_done = time.time()
                        print(f"[{datetime.datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [BLOK I] Baris {row} | IDPEL {idpel}: ID Pelanggan TIDAK DITEMUKAN di FASIH (+{t_done - t_start:.2f}s). Lanjut ke IDPEL berikutnya...")
                        sheet.cell(row=row, column=9).value = "Error : id pelanggan tidak ditemukan"
                        sheet.cell(row=row, column=10).value = time.strftime("%Y-%m-%d %H:%M:%S")
                        continue
                ...
                if not cek_label_provinsi():
                    ...
                    for swipe_up_idx in range(1, 11):
                        if cek_label_provinsi():
                            ...
                            break
                        try:
                            d.swipe(540, 500, 540, 700, duration=0.1)
                            time.sleep(0.2)
