        ...
        try:
            ...
            if cek_id_btn.exists(timeout=5):
                ...
                if d(textContains=limit_msg).exists():
                    print("PROSES BOT BERHENTI KARENA BATAS CEK ID PELANGGAN MELEWATI LIMIT")
                ...
                if not status_found:
                    ...
                    if is_already_registered:
                        print(f"[BLOK I] Baris {row} | IDPEL {idpel}: Pelanggan SUDAH TERDAFTAR di FASIH. Melewati recovery, lanjut ke IDPEL berikutnya...")
                        sheet.cell(row=row, column=9).value = "Error : SUDAH TERCATAT PADA SISTEM FASIH"
                        sheet.cell(row=row, column=10).value = time.strftime("%Y-%m-%d %H:%M:%S")
                        continue

                    if is_not_found:
                        print(f"[BLOK I] Baris {row} | IDPEL {idpel}: ID Pelanggan TIDAK DITEMUKAN di FASIH. Melewati recovery, lanjut ke IDPEL berikutnya...")
